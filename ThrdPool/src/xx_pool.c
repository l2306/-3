...
int TaskPoolDestroy()
{
//	struct lst_head* tmp = lst_pop(threadpool->pool->head.prev);
...
{
	if(NULL==tasknode){
		printf("%s %d TaskPoolAdd tasknode=NULL\n",__FILE__,__LINE__);
		return (void*)NULL;
	}

	pthread_mutex_lock(&(taskpool->mutex));		//互斥同步,保证操作的唯一性
	if(taskpool->t_curNum>=taskpool->t_maxNum){
		printf("%s %d %d task is full\n",__FILE__,__LINE__,taskpool->t_curNum);
...
void *TaskPoolDel(TaskPool* taskpool)			//删除任务(甩出 task)	//del from list ...(queue)
{
	...
	if(taskpool->t_curNum <= 0){						/* here maybe wait .....*/
		printf("%s %d %d task is empty\n",__FILE__,__LINE__,taskpool->t_curNum);
		pthread_cond_wait(&(taskpool->cond_noempty), &(taskpool->mutex));
	}
	size_t offset=((size_t) &((taskList*)0)->head);
	struct lst_head* Pos=taskpool->pool->head.prev;	//链表尾部取值
...
int ThreadPoolDestroy()
{
	...
	struct lst_head* Pos = threadpool->pool->head.prev;			//线程节点 的指针
	do{
...
//
	...
	printf("	%x",new_threadlist->thread->tid);
printf("	thr++	minIdle:%d cur:%d idle:%d maxIdle:%d\n",
...
void manager_moveTaskToWorker( ThrdPool* thrdpool )
{
	...
	if(threadpool->t_idleNum<=0)				//无空闲线程  阻塞等
...
//RUNHERE;
	if(threadpool==NULL||lst_empty(&threadpool->pool->head)) 
		return -1;
	struct lst_head* Pos=threadpool->pool->head.next;	//Pos 定位下一个空闲线程节点的指针位置
	while(Pos){
		size_t offset=((size_t) &((threadList*)0)->head);
...
int destroyThreadPool(ThrdPool* thrdpool)
{
	...
	{
		struct lst_head* Pos = threadpool->pool->head.prev;
		tmp_threadlist = (threadList*)((size_t)(char*)Pos - offset);
		Pos = (Pos->prev);
		int err = pthread_cancel(tmp_threadlist->thread->tid);
		if (err != 0)
	...
	printf("------	minIdle:%d cur:%d idle:%d maxIdle:%d\n",
		threadpool->t_minIdleNum, threadpool->t_curNum, 			threadpool->t_idleNum, threadpool->t_maxIdleNum);
}
