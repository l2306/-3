...
//仿照内核的list函数	linux内核提供用于创建双向循环链表的结构lst_head
...
//创建双向链表
static inline void INIT_lst_HEAD(struct lst_head *list)
{
        list->next = list;
        list->prev = list;
}
...
static inline void __lst_add(struct lst_head *newnode, struct lst_head *prev,struct lst_head *next)
{
        next->prev = newnode;
...
//将new添加到head后
static inline void lst_add(struct lst_head *newnode, struct lst_head *head)
{
        __lst_add(newnode, head, head->next);
}

//将new添加到 链表尾部
static inline void lst_add_tail(struct lst_head *newnode, struct lst_head *head)
{
...
//弹出 entry 节点
static inline struct lst_head* lst_pop(struct lst_head* entry)		
{
        __lst_del(entry->prev, entry->next);
        entry->next = NULL;
        entry->prev = NULL;
