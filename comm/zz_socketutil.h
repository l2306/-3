...
这就是说，当一个TCP的socket，客户端与服务端谁也不发送数据，会一直保持着连接。	（只有socket的close或程序结束时  发送FIN时才会）
这其中如果有一方异常掉线（断网线 或 掉电），另一端永远也不可能知道。这对于一些服务型的程序来说，将是灾难性的后果。
*/
...
Linux环境下的TCP Keepalive参数设置
		...
		9    
　　在Linux系统下，如果对于TCP的socket启用了Keepalive选项，则会在7200秒（即两个小时）没有数据后，发起KEEPALIVE报文
	如果没有回应，则会在75秒后再次重试。如果重试9次均失败，则认定连接已经失效。TCP的读取操作，将返回0。
	对大多数应用  前两个值有大
*/
